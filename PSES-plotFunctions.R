...
################################################################### #
Depts_vs_Scores <-  function(sortbyscore=T, includeParents=T, includeChildren=T)  {
...
################################################################### #
Questions_vs_Scores <- function(sortbyscore=T)  {
  # if (dtRESULT %>% nrow == 0)
  #   return (NULL)
  ...
  g <-  plotPSES.layout()  + 
    ...
    geom_point(data=dtRESULT[IDlevel==getLevel(myID)],mapping=aes(SCORE100, Question.Abbreviated, col=as.ordered(Theme)) , size=7, alpha=0.7) +
    
    geom_label(data=dtRESULT[IDlevel==getLevel(myID)],mapping=aes(SCORE100, Question.Abbreviated, label=RANK2) ,  alpha=0.5) +
    
    geom_point(data=dtRESULT[IDlevel==0],mapping=aes(SCORE100, Question.Abbreviated), 
               size=7,  shape=3, col="black") + # shape=15)
    
    
    geom_point(data=dtRESULT[as.integer(IDlevel)>getLevel(myID)],mapping=aes(SCORE100, Question.Abbreviated, col=as.ordered(Theme)), size=1, alpha=0.1) 
  # + 
  #   
